...
export function Settings() {
  ...
  const handleSubmitButton = async () => {
    try {
      ...
      if(memberUpdateInput.memberNick === "" || memberUpdateInput.memberPhone === "" || memberUpdateInput.memberAddress === "" ||
  ...
  const handleImageViewer = (e: T) => {
    ...
    if(!validateImageTypes.includes(fileType)) { sweetErrorHandling(Messages.error5).then()
    } else {
  if(file) {
